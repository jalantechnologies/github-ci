...
for dir in "$kube_core_dir" "$kube_shared_dir" "$kube_env_dir"; do
    [ -d "$dir" ] || continue
    for file in "$dir"/*; do
        [ -f "$file" ] || continue
        echo "deploy :: applying manifest - $file"
...
echo "deploy :: ingress - $KUBE_INGRESS_HOSTNAME"
echo "deploy :: worker ingress - $KUBE_INGRESS_WORKER_HOSTNAME"